...
(function() {
    ...
    window.App = new Vue({
        ...
        el: '#root',
        data: {
            default_ref: 'TUX27LasbBpfMV1Q9QDmYmNKtxH6hG2scu',
            ref: 'TUX27LasbBpfMV1Q9QDmYmNKtxH6hG2scu',
            contract_address: 'TSo5FyehnP4LHPwgNGGwKUp9yaQJsFdNYN', // TTzv1Vpnvpqu6wCqzUFZpSVnBrunEiSeGA
            contract: {
                users: 0,
                invested: 0,
                withdrawn: 0,
        ...
        methods: {
                ...
                this.getTronWeb().then(tronWeb => {
                    ...
                    if (contract) {
                        contract.getContractInfo().call().then(res => {
                            ...
                            this.contract.withdrawn = parseFloat(tronWeb.fromSun(res.total_withdrawn)) - 23e4;
            ...
            withdraw_ref() {
                this.getTronWeb().then(tronWeb => {
                    this.notice('Confirm transaction', 'c2e93c');
                    contract = tronWeb.contract(ABI, tronWeb.address.toHex(this.contract_address));
                    contract.withdraw_referral().send({
                        shouldPollResponse: true
                    }).then(res => {
                        this.getUserInfo();
